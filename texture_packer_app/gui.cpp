...
public:
  TexturePackerGuiApplication()
  {
    ...
    m_window =
        WindowPtr(SDL_CreateWindow("Texture packer", 1280, 900, window_flags), SDL_DestroyWindow);

    if (m_window == nullptr)
    {
      throw std::runtime_error(fmt::format("Error: SDL_CreateWindow(): {}", SDL_GetError()));
    }
    m_renderer = RenderPtr(SDL_CreateRenderer(m_window.get(),
                                              nullptr,
                                              SDL_RENDERER_PRESENTVSYNC | SDL_RENDERER_ACCELERATED),
                           SDL_DestroyRenderer);
    if (m_renderer == nullptr)
    {
      throw std::runtime_error(fmt::format("Error: SDL_CreateRenderer(): {}", SDL_GetError()));
    }
  ...
  int Run() &&
  {
    while (m_is_running)
    {
      ...
      SDL_SetRenderDrawColor(m_renderer.get(), (Uint8)(20), (Uint8)(20), (Uint8)(20), (Uint8)(255));
...
private:
  ...
  void ProcessHierarchy()
  {
        ...
        ImGui::CollapsingHeader(m_selected_folder.string().c_str()))
    {
      ...
      if (old_path != m_selected_path)
      {
        ...
        static const std::set<std::string> kSupportedFormats = {".png", ".jpg", ".jpeg", ".bmp"};
        if (kSupportedFormats.contains(m_selected_path.extension().string()))
        {
          m_image_to_preview = std::make_unique<TexturePacker::CImage>(m_selected_path.string());
        }
        else
        {
          m_image_to_preview.release();
  ...
  void ProcessPreview()
  {
    ...
    if (m_image_to_preview != nullptr)
    {
      ...
      float scale = std::min(windows_size.x / texture_size.x, windows_size.y / texture_size.y);
      if (scale > 1.f)
      {
        scale = 1.f;
      }
      ...
      cursor_pos =
          ImVec2{(windows_size.x - texture_size.x) / 2.f, (windows_size.y - texture_size.y) / 2.f};
